...
def cross_validate(input_, output, folds, partitions, seeds, region, domain_fp,
    ...
    def fit_rf(sp, partition, sample_weight, seeds, partition_counter):
        clf = RandomForestClassifier(n_estimators=rf_n_estimators)
        sp_clf = SpatialClassifier(clf, partition)
        sp_clf.fit(sp, sample_weight=sample_weight)
        return sp_clf
        
    def fit_nn(sp, partition, sample_weight, seeds, partition_counter):
        model = Sequential()
    ...
    for fold in range(folds):
        ...
        if seeds == 'none':
            ...
            print('Train areas: {} Test areas: {} Centroids: {}'.format(sp_train.areas, sp_test.areas, centroids))
...
if __name__ == '__main__':
    ...
    args.area = [_[0:len(_)] for _ in args.area]
    print(args.region)
